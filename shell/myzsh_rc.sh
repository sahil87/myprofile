...
path=($path . ~/.bin ~/.rvm/bin ~/.local/bin /snap/bin $CODEDIR/gmetrivr/dec/bin $MYPROFILEDIR/bin ~/.yarn/bin /usr/local/go/bin ~/code/bin ~/code/go/bin ~/code/concourse/bin ~/Android/Sdk/platform-tools ~/Android/Sdk/tools /mnt/files/storage/groovy/bin $GRADLE_USER_HOME/bin)
